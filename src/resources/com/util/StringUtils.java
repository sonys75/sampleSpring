...
public class StringUtils {
    ...
    public static String dateToString(Date date, String fmt) {
        ...
        try {
            java.text.SimpleDateFormat sDate = new java.text.SimpleDateFormat(fmt, Locale.ENGLISH);
    ...
    public static Date stringToDate(String date, String fmt) {
        // "yyyy-MM-dd HH:mm:ss"
        try {
            java.text.SimpleDateFormat sdfDate = new java.text.SimpleDateFormat(fmt, Locale.ENGLISH);
     ...
     * str에서 one을 two로 바꾼다.
     **/
    public static String replace(String str, String one, String two) {
        if (str == null) {
            return str;
        }

        StringTokenizer st = new StringTokenizer(str, one, false);
        String res = "";
    ...
    public static String toScriptString(String str) {
        ...
        ret = ret.replace((char) 13, (char) 0);
        ret = ret.replace((char) 10, (char) 0);
        ret = replace(ret, "\'", "\\'");
        return ret;
    }
    ...
    /**
     * 주어진 String의 크기가 주어진 크기코다 작으면 나머지는 " "로 채운다.
	 ...
	 * @return
	 */
	public static String AppendZero(String strNum){
		String strResult = "";
		if( strNum != null && !"".equals(strNum) ){
			if(isNumeric(Trim(strNum)) == true){
	 ...
	 */
	public static boolean isNumeric(String lst_str)
	{
		boolean lb_result = true; // 먼저 숫자형 데이터로만 이루어져 있다고 가정한다.
		String lst_tmpstr = getNull(lst_str);
		...
		{
			int li_strlen = lst_tmpstr.length();
			for(int i =0; i < li_strlen; i++)
			{
				char a = lst_tmpstr.charAt(i);
