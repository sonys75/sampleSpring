...
<%@ taglib prefix="spring" uri="http://www.springframework.org/tags"%>
				...
				<div id="content">
					<div class="navi">홈 &gt; 사이트관리 &gt; 게시판관리</div>
					<form name="F1" id="F1" method="post" action="/mng/brd/list.do" >
			        	<!-- 검색 Str -->
		                <table class="tbl">
			                ...
			                <c:choose>
			                	<c:when test="${not empty list}">
			                        ...
			                        <tr>
			                            ...
			                            <a href="/mng/brd/view.do?seq=${result.seq}&page_no=${listPage.currentPage}<c:if test="${not empty listPage.commonPara}">&${listPage.commonPara}</c:if>"><c:out value="${result.subject}"/></a>
			                            </td>
			            ...
			            <div class="btnR">
			            	<a class="button" title="등록" href='/mng/brd/regist.do?page_no=${listPage.currentPage}<c:if test="${not empty listPage.commonPara}">&${listPage.commonPara}</c:if>'>등록</a>
			            </div> 
					</form>
				</div>
