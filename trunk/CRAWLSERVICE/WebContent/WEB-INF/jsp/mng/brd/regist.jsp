...
<script type="text/javascript">
	function fncSave() {
		...
		*/
		...
		f.action = "/mng/brd/registProc.do";
		f.submit();
		f.target = "_self";
		f.encoding = "application/x-www-form-urlencoded";
		f.action = "/mng/brd/regist.do";
		return true;
	}
				...
				<form:form commandName="VoBoard" name="F1" id="F1" method="post" onsubmit="return fncSave();" enctype="multipart/form-data">
					...
					<div class="btnC">
						...
						<c:if test="${VoBoard.mode=='U'}">
							<a class="button" href="/mng/brd/view.do<c:if test="${not empty retParam}">?${retParam}&seq=${VoBoard.seq}</c:if>" title="취소">취소</a>
						...
						<a class="button" href="/mng/brd/list.do<c:if test="${not empty retParam}">?${retParam}</c:if>" title="목록">목록</a>
					</div>
				</form:form>
			</div>
